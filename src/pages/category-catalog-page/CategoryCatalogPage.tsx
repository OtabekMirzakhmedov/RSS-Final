...
import { Categories } from '../pages-types/pageTypes';
import { GetCategoryProducts, MainPageProduct } from '../../service/ProductService';
import './categoryCatalogPage.scss';

function CategoryCatalogPage() {
  ...
  if (categoryName === 'biography') {
    catalogId = Categories.biography;
  } else {
    catalogId = Categories.fiction;
  }

  const fetchProducts = async (sortOption: string, searchQuery: string) => {
    setLoading(true);
    try {
      const fetchedProducts = await GetCategoryProducts(catalogId, sortOption, searchQuery);
      if (fetchedProducts) {
